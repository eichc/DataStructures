...
void StudentTest(){
	...
	//testing get and set
	double d;
	assert(m1.get(-1,0,d) == false);
	assert(m1.get(0,10,d) == false);
	assert(m1.set(-1,0,d) == false);
	assert(m1.set(0,10,d) == false);

	//testing get_row and get_col
	double* r1 = m1.get_row(2);
	double* r2 = m1.get_row(5);
	double* c1 = m1.get_col(3);
	double* c2 = m1.get_col(-1);
	
	assert(r1[2] == 2 && c1[2] == -4);
	assert(r2 == NULL && c2 == NULL);
	...
	//testing quarter() edge case
	Matrix m2;
	Matrix* m2_q = NULL;
	m2_q = m2.quarter();

	std::cout << "Quarter with empty matrix:" << std::endl;
	std::cout << "UL: " << std::endl << m2_q[0] << std::endl;
	std::cout << "UR: " << std::endl << m2_q[1] << std::endl;
	std::cout << "LL: " << std::endl << m2_q[2] << std::endl;
	std::cout << "LR: " << std::endl << m2_q[3] << std::endl;
...
void ExtraCreditTest(){
	...
	Matrix m1(3,3,0);
	std::cout << "Original m1:" << std::endl << m1 << std::endl;

	m1.resize(5,7,1);
	std::cout << "m1 resized to 5x7:" << std::endl << m1 << std::endl;

	m1.resize(6,6,2);
	std::cout << "m1 resized to 6x6:" << std::endl << m1 << std::endl;

	m1.resize(3,3,3);
	std::cout << "m1 resized to 3x3:" << std::endl << m1 << std::endl;

	m1.resize(0,0,4);
	std::cout << "m1 resized to 0x0:" << std::endl << m1 << std::endl;
}
