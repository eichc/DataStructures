...
void print_grid(const std::vector<std::vector<GRID_STATUS> > & blocked_grid,
                unsigned int start_x, unsigned int start_y, std::list<Point>& path) {
...
void find_paths(const std::vector<std::vector<GRID_STATUS> > & blocked_grid, 
  std::list<Point>& path, int x, int y, bool found) {
  ...
  //if at a dead end
  if ((x-1 < 0 || blocked_grid[x-1][y] == GRID_BLOCKED) && 
    (y-1 < 0 || blocked_grid[x][y-1] == GRID_BLOCKED)) {
    path.pop_back();
  }

  else {
    if (x-1 >= 0 && blocked_grid[x-1][y] == GRID_CLEAR) {
      find_paths(blocked_grid, path, x-1, y, found);
    }
    if (y-1 >= 0 && blocked_grid[x][y-1] == GRID_CLEAR) {
      find_paths(blocked_grid, path, x, y-1, found);
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  std::vector<std::vector<GRID_STATUS> > blocked_grid_copy = blocked_grid;
  bool found = false;
  find_paths(blocked_grid_copy, path, start_x, start_y, found);
  ...
  print_grid(blocked_grid_copy, start_x, start_y, path);
