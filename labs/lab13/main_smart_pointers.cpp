...
void findNodes(Balloon* b, std::set<Balloon*>& nodes) {
  std::pair<std::set<Balloon*>::iterator, bool> result = nodes.insert(b);
  if (!result.second) {
    return;
  }
  for (int i = 0; i < b->numRopes(); i++) {
    findNodes(b->getRope(i), nodes);
  }
}
...
void deleteAll(Balloon* b) {
  ...
  findNodes(b, nodes);
  for (std::set<Balloon*>::iterator itr = nodes.begin(); itr != nodes.end(); ++itr) {
    delete *itr;
  }
}
